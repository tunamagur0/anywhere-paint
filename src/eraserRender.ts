...
export default class EraserRender implements PenInterface {
  private pre: {
    x: number;
    y: number;
    pressure: number;
  ...
  start(
    ...
    this.ctx.lineWidth = EraserRender.getWidth(
      history.info.lineWidth,
      info.pressure
    );

    this.history.info.path.push(info);
    this.history.info.mode = 'Eraser';
  ...
  update(info: { x: number; y: number; pressure: number }): void {
    ...
    this.ctx.lineWidth = EraserRender.getWidth(this.pre.width, info.pressure);
    this.ctx.beginPath();
